...
class TestProjectCommands:
    ...
    def test_command_init_w_template_no_prompt_runs_successfully(mldock_config_aws):
        ...
        assert isinstance(mldock_config.pop('hyperparameters'), dict), "Failed. hyperparameters was not a dict"
        assert isinstance(mldock_config.pop('model'), list), "Failed. model was not a list"
        assert isinstance(mldock_config.pop('stages'), dict), "Failed. stages was not a dict"
    ...
    def test_command_init_w_template_no_prompt_runs_fails_gracefully(mldock_config_aws):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            assert not mldock_filepath.is_file(), "Failed. A mldock.json was found"

    @staticmethod
    def test_command_init_w_template_no_prompt_w_param_runs_successfully(mldock_config_aws):
        """
            Tests that the init with user input command runs successfully.
            Does not test that files are copied correctly.
        """
        ...
        assert mldock_config.pop('environment') == {'my_var': 'value'}, "Failed. environment were not correct."
        assert mldock_config.pop('hyperparameters') == {'my_param': 'value'}, "Failed. hyperparameters were not correct."
        assert mldock_config == mldock_config_aws, "Failed. MLDOCK container project config was incorrect"
