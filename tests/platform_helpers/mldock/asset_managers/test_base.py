...
class ExampleLocalEnvArtifactManager(BaseEnvArtifactManager):
    ...
    def upload_assets(self, **kwargs):
        file_system = fs.LocalFileSystem()
        upload_assets(file_system, **kwargs)
...
class TestBaseEnvArtifactManager:
    ...
    def test_setup_inputs_success(self):
        ...
        with tempfile.TemporaryDirectory() as result_tempdir:
            ...
            assert Path(
                result_tempdir, "input/data/example/data.txt"
            ).exists(), "Failure."
    ...
    def test_cleanup_outputs_success(self):
        ...
        with tempfile.TemporaryDirectory() as result_tempdir:
            ...
            env_vars = {
                "MLDOCK_OUTPUT_CHANNEL_EXAMPLE": Path(
                    result_output_data_path, "data.txt"
                ).as_posix(),
            ...
            assert Path(
                output_tempdir.name, "data/example/data.txt"
            ).exists(), "Failure."
            assert Path(
                output_tempdir.name, "model/example/artifacts.zip"
            ).exists(), "Failure."
