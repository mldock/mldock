...
def format_key_as_mldock_env_var(key, prefix=None):
    """
...
def format_dictionary_as_env_vars(obj: dict, group=None):
    """
    ...
    for _key, _value in obj.items():
        ...
        _key = format_key_as_mldock_env_var(_key, prefix=group)

        if isinstance(_value, dict):
            _value = json.dumps(_value)

        new_keys.update({_key: _value})
    return new_keys


def collect_mldock_environment_variables(**env_vars):
    """Collect and format mldock environment variables"""
    return format_dictionary_as_env_vars(env_vars, group="mldock")
...
def extract_data_channels_from_mldock(mldock_data: list):
    """extracts data channels as sagemaker input data configs"""
    data_channels = [data_config["channel"] for data_config in mldock_data]

    return make_sagemaker_input_data_configs(data_channels=data_channels)
